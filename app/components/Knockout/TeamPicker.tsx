...
export function TeamPicker({ matchId, teams, onSelect, onClose }: Props) {
  ...
  const filtered = query.trim()
    ? eligible.filter(t => t.name.toLowerCase().includes(query.toLowerCase()))
    : eligible

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={onClose}>
      <div
        className="bg-gray-900 border border-gray-700 rounded-xl p-4 w-80 max-h-[70vh] flex flex-col shadow-2xl"
      ...
      >
        ...
        <input
          ...
          className="mb-3 w-full bg-gray-800 border border-gray-600 rounded px-3 py-1.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-gray-400"
        />
        <div className="overflow-y-auto flex-1">
          {eligible.length === 0 ? (
            <p className="text-gray-400 text-sm">No eligible teams. Complete the Group Stage first.</p>
          ) : filtered.length === 0 ? (
            <p className="text-gray-500 text-sm">No teams match "{query}".</p>
          ) : (
